      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-card p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={data.by_bedrooms || []}>
              <XAxis dataKey="bedrooms" stroke="#6b7280" />
              <YAxis stroke="#6b7280" tickFormatter={v => `$${v/1000}K`} />
              <Tooltip formatter={v => [`$${(v/1000).toFixed(0)}K`, 'Avg Price']} contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: '12px' }} />
        ...
        <div className="glass-card p-6">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={data.price_distribution || []} nameKey="bucket" dataKey="c" cx="50%" cy="50%" innerRadius={60} outerRadius={90} label>
                {(data.price_distribution || []).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
        ...
        <div className="glass-card p-6 lg:col-span-2">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={(data.by_year || []).slice(-20)}>
              <XAxis dataKey="year_built" stroke="#6b7280" />
              <YAxis stroke="#6b7280" tickFormatter={v => `$${v/1000}K`} />
              <Tooltip formatter={v => [`$${(v/1000).toFixed(0)}K`, 'Avg']} contentStyle={{ background: '#1f2937', border: '1px solid #374151', borderRadius: '12px' }} />
